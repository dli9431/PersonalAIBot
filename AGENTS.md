...
This repository is a self-hosted Discord bot that turns Discord messages into coding runs executed by Claude Code or Codex CLI, then manages the resulting git workflow.
...
The bot is designed for Linux and WSL2. It expects the external CLIs `claude`, `codex`, and optionally `gh` to be installed on the host machine.
...
3. Login helpers for Claude and Codex CLI auth flows.
...
- Follow-up messages continue the current engine session with Claude resume or Codex resume.
...
- Task execution: plain text, `claude:`, `codex:`, `cc:`, `cx:`, `openai:`
...
- Runtime config: `engine`, `engine global`, `claude models`, `codex models`, `claude model`, `codex model`, reasoning commands
