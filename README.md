...
| `usage` | Show cumulative token usage per engine (all-time runs, input/output/cache tokens); also shows current session token count if a Claude session is active |
