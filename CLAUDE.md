...
2. **Helpers** (~lines 99-278) — Auth check, slugify, `run_git(cmd, path)` with optional cwd override, `resolve_project`, `resolve_branch` (N/#N ref lookup), `get_diff`/`get_diff_stat` (compares feature branch vs base, not just working tree), `branch_merged_status`.
...
- **Branch switching**: `switch <branch|N>` mid-session auto-commits and checks out the target branch. `N`/`#N` refs are populated by the `branches` command via `branch_listing` dict.
